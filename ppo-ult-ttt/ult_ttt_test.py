...
class TestState(TestCase):
    ...
    def assertStatesEqual(self, state1: ImmutableState, state2: ImmutableState):
        self.assertTrue(np.all(state1.board == state2.board))
        self.assertEqual(state1.fill_num, state2.fill_num)
    
    def test_board_string_to_state(self):
        boards, moves = generate_game_history()
        fill_num = 1
        state_str = convert_board_to_string(boards[0].board)
        self.assertStatesEqual(boards[0], board_string_to_state(state_str, None))
        fill_num = get_next_turn_fill_num(fill_num)
        for state, action in zip(boards[1:], moves):
            _, _, j, k = action
            state_str = convert_board_to_string(state.board)
            self.assertStatesEqual(state, board_string_to_state(state_str, (j, k)))
            fill_num = get_next_turn_fill_num(fill_num)
