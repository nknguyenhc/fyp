...
def board_string_to_state(board_string: str, prev_local_action: LocalBoardAction) -> ImmutableState:
    local_board_rows = board_string.strip().split(horizontal_separator)
    board = np.zeros((3, 3, 3, 3))
    count = 0
    for i, row in enumerate(local_board_rows):
        for j, sub_row in enumerate(row.strip().split('\n')):
            for m, local_board_row in enumerate(sub_row.strip().split('|')):
                for n, cell in enumerate(local_board_row.strip().split('  ')):
                    match cell.strip():
                        case 'X':
                            board[i][m][j][n] = 1
                            count += 1
                        ...
                        case '-':
                            board[i][m][j][n] = 0
                        case _:
                            raise ValueError(f'Unrecognised cell: "{cell.strip()}"')
    turn = 1 if count % 2 == 0 else 2
    return ImmutableState(board=board, fill_num=turn, prev_local_action=prev_local_action)
