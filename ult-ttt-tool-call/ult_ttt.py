...
def convert_string_to_board(board_str: str) -> np.ndarray:
    board = np.zeros((3, 3, 3, 3), dtype=int)
    lines = [line for line in board_str.splitlines() if line.strip() and "---" not in line.strip()]
    for i in range(3):
        for j in range(3):
            rows = lines[i * 3 + j].split('|')
            for k in range(3):
                # Parse into an array of non-space characters
                cells = []
                for char in rows[k]:
                    if char != ' ':
                        cells.append(char)
                for l in range(3):
                    cell_str = cells[l].strip()
                    if cell_str == 'X':
                        board[i][k][j][l] = 1
                    elif cell_str == 'O':
                        board[i][k][j][l] = 2
...
def get_valid_moves(state: str, prev_move: int) -> list[int]:
    """
    Given the string representation of the board state and the previous move, return the list of valid moves (1-81).
    Example of state string:
    X  X  X  | -  O  O  | -  O  -
    -  -  -  | -  X  X  | X  O  X
    -  -  -  | -  O  O  | O  X  O
    ------------------------------
    ...
    Args:
        state: The string representation of the board state.
        prev_move: The previous move (1-81).
    Returns:
        A list of valid moves (1-81).
    """

    try:
        board = convert_string_to_board(state)
    except:
        raise ValueError("Invalid board state string")
    prev_move = prev_move - 1
    i = (prev_move % 9) // 3
    j = (prev_move % 9) % 3
    # Determine turn
    ones_count = int(np.sum(board == 1))
    twos_count = int(np.sum(board == 2))
    fill_num = 1
    if ones_count > twos_count:
        fill_num = 2
    state = ImmutableState(board=board,
                            prev_local_action=(i, j),
                            fill_num=fill_num)
    actions = get_all_valid_actions(state)
    valid_moves = []
    for action in actions:
        move_num = 27 * action[0] + 9 * action[1] + 3 * action[2] + action[3] + 1
        valid_moves.append(move_num)
    return valid_moves
